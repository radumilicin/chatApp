...
app.post('/sendMessage', async (req, res) => {
  try {
    ...
    console.log("In send message (no websocket) before check")  
      
    if (!sender_id || !recipient_id || !timestamp || !ciphertext || !ciphertext_sender) {
      return res.status(400).json({ error: 'Invalid message format' });
    }
    
    console.log("In send message after initial check")  
    ...
    console.log("Before inserting message in DB")  
    ...
    console.log("After inserting message in DB")  
