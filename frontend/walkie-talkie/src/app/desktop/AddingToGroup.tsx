...
export default function AddPersonToGroup(props) {
    ...
    }, [props.decryptedContacts, props.users])

    async function changeFilteredContacts () {
        if(props.decryptedContacts !== null || props.decryptedContacts !== undefined) {
            ...
            for(let user of users_matching_search) {
                for(let contact of props.decryptedContacts){
    ...
    async function insertMembersInGroup() {
        ...
        if(props.decryptedContacts === null) return
    ...
    return (
        <div className={`absolute left-[30%] w-[40%] top-[10%] h-[80%] rounded-2xl overflow-hidden
