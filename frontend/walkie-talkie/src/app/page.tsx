...
export default function Home() {
  ...
  const fetchData = async () => {
      const response = await fetch(`${API_URL}/users`); // gets all users (need this for when adding contacts)
  ...
  const fetchData2 = async () => {
      const response = await fetch(`${API_URL}/contacts?user=${user}`); // gets contacts of current user
      const result = await response.json();
  ...
  const fetchImages = async () => {
      const response = await fetch(`${API_URL}/images`); // Gets all images (TODO images of all contacts and users)
      const result = await response.json();
      updateImages(result);
      // console.log(result);
    };
  ...
  useEffect(() => {
    ...
    const checkKeys = async () => {
      ...
      // Check database
      try {
        const dbResponse = await fetch(`${API_URL}/api/keys?recipient_id=${user}`);
        if (!dbResponse.ok) {
  ...
  async function closeChat(contact) {
    ...
    try { 
      const resp = await fetch(`${API_URL}/closeChat`, {
        method: "PUT",
        credentials: "include",
        headers: {
  ...
  async function sendMessageStatusUpdate(timestamp: any, status: string, contact_id: string) {
    ...
    const response = await fetch(`${API_URL}/updateMessageStatus`, { 
      method: "PUT",
      credentials: "include",
      headers: {
        "Content-Type": "application/json"
  ...
  async function loadConversationRatchetStateDB(user, contact) {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/getRatchetState?user_id=${user}&conversation_id=${contact.id}`
      );
        ...
        {loggedIn === true && <div className={`relative left-0 top-0 w-full h-full flex flex-row bg-gradient-to-br ${themeChosen === "Dark" ? "from-[#181c25] via-[#01050c] to-[#0d1018]" : "from-[#258ebe] via-[#7823a8] to-[#10bb79]"}
          ...
          {pressedProfile ? (display === "Desktop" ? <ProfileSettings users={users} curr_user={user} images={images} setPressProfile={setPressProfile} fetchData={fetchData} 
            ...
            curr_contact !== null && profileInfo === false && display === "Mobile" ? <CurrentChatVertical users={users} contacts={contacts} images={images} contact={curr_contact} curr_user={user} setProfileInfo={setProfileInfo} 
                                                ...
                                                identityKey={identityKey} signedPreKey={signedPreKey} decryptAllMessages={decryptAllMessages} decryptedContacts={decryptedContacts} setCurrContact={setCurrContact} 
                                                loadConversationRatchetStateDB={loadConversationRatchetStateDB} sendMessageStatusUpdate={sendMessageStatusUpdate} disappearingMessagesPeriod={disappearingMessagesPeriod} isConnected={isConnected}></CurrentChatVertical>
                                    :
          ...
          {profileInfo === false ? (display === "Desktop" ? <CurrentChat users={users} contacts={contacts} images={images} contact={curr_contact} curr_user={user} setProfileInfo={setProfileInfo} 
                                                ...
                                                loadConversationRatchetStateDB={loadConversationRatchetStateDB} sendMessageStatusUpdate={sendMessageStatusUpdate} disappearingMessagesPeriod={disappearingMessagesPeriod} isConnected={isConnected}></CurrentChat> : <></>)
                                : (display === "Desktop" ? <ProfileInfo setProfileInfo={setProfileInfo} contact={curr_contact} users={users} curr_user={user} contacts={contacts} images={images} fetchContacts={fetchData2} fetchUsers={fetchData} 
                                      fetchImages={fetchImages} setCurrContact={setCurrContact} setAddToGroup={setAddToGroup} addingToGroup={addingToGroup} themeChosen={themeChosen} setDecryptedContacts={setDecryptedContacts}></ProfileInfo> : <></>) }
        </div>
        }
